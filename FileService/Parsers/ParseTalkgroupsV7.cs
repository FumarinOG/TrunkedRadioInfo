...
{
    public class ParseTalkgroupsV7 : ITalkgroupParser
    {
        ...
        private const int COL_ENCRYPTION_SEEN = 9;
        private const int COL_HITS = 10;
        private const int COL_IGNORE_EMERGENCY_SIGNAL = 11;

        private readonly int _systemID;

        public ParseTalkgroupsV7(int systemID)
        {
            _systemID = systemID;
        }
        ...
        public Talkgroup ParseTalkgroup(string[] row)
        {
            ...
            talkgroup.EncryptionSeen = row[COL_ENCRYPTION_SEEN].Equals("Y", StringComparison.OrdinalIgnoreCase);
            talkgroup.HitCountProgram = int.Parse(row[COL_HITS]);
            talkgroup.IgnoreEmergencySignal = row[COL_IGNORE_EMERGENCY_SIGNAL].Equals("1",
